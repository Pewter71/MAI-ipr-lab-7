...
## 3. Проверка критерия 1 - работа без OTLP endpoint

Остановить стек и запустить только базовый compose (без observability):
...
Проверить работу через exec (порт 3002 не проброшен без overlay):

```powershell
docker exec chat-app-backend sh -c "curl -s http://localhost:3001/health"
docker exec chat-app-backend sh -c "curl -s http://localhost:3001/metrics"
```

Проверить что в логах нет ошибок OTLP:
...
## 5. Проверка критерия 2 - scrape метрик
...
## 6. Проверка критерия 3 - Grafana: метрики и трейс
...
Скриншоты хранятся в docs/screenshots/lab7/